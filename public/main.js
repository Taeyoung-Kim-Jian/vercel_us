...
document.addEventListener("DOMContentLoaded", async () => {
      ...
      <ul class="top5-list">
        ${top5
          .map(
            (r, i) => `
          <li class="top5-item">
            <span class="name">${i + 1}. ${r.종목명 || "-"}</span>
  ...
  async function updateSwingPriceTable() {
    ...
    swingTbody.innerHTML = `<tr><td colspan="4">🧮 계산 중...</td></tr>`;

    try {
      const { data: bData } = await db.from("bt_points").select("종목코드, b가격");
      ...
      swingData = results;
      renderSwing();
      console.log(`✅ ${results.length}개 종목이 swing_price 계산됨`);
    } catch (error) {
      console.error("❌ 스윙 계산 오류:", error);
      swingTbody.innerHTML = `<tr><td colspan="4">❌ 계산 실패</td></tr>`;
    }
  }
